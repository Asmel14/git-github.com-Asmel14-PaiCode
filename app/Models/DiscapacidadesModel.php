...
class DiscapacidadesModel extends BaseModel
{
    protected string $table = 'discapacidades';
    ...
    {
        $sql = 'SELECT * FROM `discapacidades` WHERE `estado` = 1 ORDER BY `nombre` ASC';
    ...
    {
        $sql = 'SELECT * FROM `discapacidades` ORDER BY `nombre` ASC, `id` DESC';
        $stmt = $this->db->query($sql);

        return $stmt->fetchAll();
    }

    public function createDiscapacidad(array $data): int
    {
        $this->validateData($data);

        $nombre = trim((string) $data['nombre']);
        if ($this->getByNombre($nombre) !== null) {
            throw new InvalidArgumentException('Ya existe una discapacidad con ese nombre.');
        }
        ...
        if ($newId === null) {
            throw new RuntimeException('No se pudo crear la discapacidad.');
        }

        return $newId;
    }

    public function updateDiscapacidad(int $id, array $data): int
    {
        $this->validateData($data, true);

        $current = $this->getById($id);
        if ($current === null) {
            throw new InvalidArgumentException('La discapacidad indicada no existe.');
        }
