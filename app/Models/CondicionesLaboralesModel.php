...
class CondicionesLaboralesModel extends BaseModel
{
    protected string $table = 'condiciones_laborales';
    ...
    {
        $sql = 'SELECT * FROM `condiciones_laborales` WHERE `estado` = 1 ORDER BY `nombre` ASC';
    ...
    {
        $sql = 'SELECT * FROM `condiciones_laborales` ORDER BY `nombre` ASC, `id` DESC';
        $stmt = $this->db->query($sql);

        return $stmt->fetchAll();
    }

    public function createCondicion(array $data): int
    {
        $this->validateData($data);

        $nombre = trim((string) $data['nombre']);
        if ($this->getByNombre($nombre) !== null) {
            throw new InvalidArgumentException('Ya existe una condicion laboral con ese nombre.');
        }
        ...
        if ($newId === null) {
            throw new RuntimeException('No se pudo crear la condicion laboral.');
        }

        return $newId;
    }

    public function updateCondicion(int $id, array $data): int
    {
        $this->validateData($data, true);

        $current = $this->getById($id);
        if ($current === null) {
            throw new InvalidArgumentException('La condicion laboral indicada no existe.');
        }
