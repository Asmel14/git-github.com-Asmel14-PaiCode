...
} elseif (isset($_FILES['foto_personal']) && is_array($_FILES['foto_personal'])) {
    $fotoInput = $_FILES['foto_personal'];
}
...
$relativeDir = 'uploads/personal';
$uploadDir = $publicRoot . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'personal';
if (!is_dir($uploadDir) && !mkdir($uploadDir, 0775, true) && !is_dir($uploadDir)) {
    respond(500, false, 'No se pudo crear el directorio de fotos de personal.');
}

$extension = $allowedMimes[$mime];
$filename = 'personal_' . date('Ymd_His') . '_' . bin2hex(random_bytes(4)) . '.' . $extension;
